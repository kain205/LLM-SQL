...
def get_indexing_pipeline(doc_store: PgvectorDocumentStore):
    ...
    """
    # Converts Markdown files into Haystack Documents
    converter = MarkdownToDocument()
    
    # Cleans up and splits the Markdown documents
    preprocessor = RecursiveDocumentSplitter(
        separators= ["\n\n", "\n", " "],
    #split_length=500,              # Độ dài tối đa mỗi khối (chunk) tính bằng ký tự
    #split_overlap=50,              # Số ký tự gối lên nhau giữa các khối
    #split_unit="char"              # Đơn vị đo là ký tự (character)
    )
...
def run_indexing(pipeline: Pipeline, knowledge_base_path="knowledge.md"):
    """
    ...
    pipeline.run({"converter": {"sources": [knowledge_base_path]}})
